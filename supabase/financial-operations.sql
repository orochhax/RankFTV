...
-- Estornos que o provedor encerrou de forma inequívoca podem receber uma
-- única nova referência lógica. Estados pendentes/ambíguos nunca entram aqui:
-- eles continuam na mesma operação e são conciliados, sem novo estorno.
...
REVOKE ALL ON FUNCTION financial_resolve_refund_reference(text,uuid,text) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION financial_resolve_refund_reference(text,uuid,text) TO service_role;
