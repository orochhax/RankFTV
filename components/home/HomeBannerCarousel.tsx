...
export function HomeBannerCarousel({ banners }: { banners: HomeBanner[] }) {
  ...
  return (
    <section aria-roledescription="carrossel" aria-label="Banners da home" className="group relative touch-pan-y overflow-hidden rounded-3xl bg-gray-100 shadow-sm ring-1 ring-black/5" onMouseEnter={carousel.pauseOnHover} onMouseLeave={carousel.resume} onKeyDown={(event) => { if (event.key === "ArrowLeft") carousel.previous(); if (event.key === "ArrowRight") carousel.next(); }} {...carousel.pointerHandlers} {...carousel.focusHandlers}>
