...
export type ChampionshipReviewSummary = {
  name: string;
  dateLabel: string;
...
function validateAthleteFields(
  values: Record<string, string>,
  useSameEmail: boolean,
): Partial<Record<ComprarAtletaField, string>> {
  const buyerName = (values.comprador_nome ?? "").trim();
  const buyerCpf = normalizeCpf(values.comprador_cpf ?? "");
  const buyerEmail = (values.comprador_email ?? "").trim().toLowerCase();
  const buyerEmailConfirmation = (values.comprador_email_confirmacao ?? "")
    .trim()
    .toLowerCase();
  const buyerWhatsApp = (values.comprador_zap ?? "").replace(/\D/g, "");
  const partnerName = (values.parceiro_nome ?? "").trim();
  const partnerCpf = normalizeCpf(values.parceiro_cpf ?? "");
  const partnerEmail = (values.parceiro_email ?? "").trim().toLowerCase();
  const partnerEmailConfirmation = (values.parceiro_email_confirmacao ?? "")
    .trim()
    .toLowerCase();
  const partnerWhatsApp = (values.parceiro_zap ?? "").replace(/\D/g, "");
  const errors: Partial<Record<ComprarAtletaField, string>> = {};

  if (!isValidAthleteName(buyerName) || buyerName.length < 2)
    errors.comprador_nome = "Informe seu nome completo.";
  if (!validaCPF(buyerCpf))
    errors.comprador_cpf = "CPF inválido. Confira os números informados.";
  if (buyerWhatsApp && !/^\d{10,15}$/.test(buyerWhatsApp))
    errors.comprador_zap = "Informe o WhatsApp com DDD e número.";
  if (!EMAIL_PATTERN.test(buyerEmail))
    errors.comprador_email =
      "Informe um e-mail válido para acessar o ingresso.";
  if (!buyerEmailConfirmation || buyerEmailConfirmation !== buyerEmail)
    errors.comprador_email_confirmacao =
      "A confirmação precisa ser igual ao e-mail do atleta 1.";
  if (!values.comprador_genero)
    errors.comprador_genero = "Selecione o gênero do atleta 1.";

  if (!isValidAthleteName(partnerName) || partnerName.length < 2)
    errors.parceiro_nome = "Informe o nome do parceiro.";
  if (!validaCPF(partnerCpf))
    errors.parceiro_cpf =
      "CPF do parceiro inválido. Confira os números informados.";
  else if (buyerCpf === partnerCpf)
    errors.parceiro_cpf = "O CPF do parceiro não pode ser igual ao seu.";
  if (partnerWhatsApp && !/^\d{10,15}$/.test(partnerWhatsApp))
    errors.parceiro_zap = "Informe o WhatsApp com DDD e número.";
  if (!EMAIL_PATTERN.test(partnerEmail))
    errors.parceiro_email =
      "Informe o e-mail do parceiro para ele acessar o próprio ingresso.";
  else if (!useSameEmail && partnerEmail === buyerEmail)
    errors.parceiro_email =
      "Use um e-mail diferente para cada atleta receber sua própria credencial.";
  if (!partnerEmailConfirmation || partnerEmailConfirmation !== partnerEmail)
    errors.parceiro_email_confirmacao =
      "A confirmação precisa ser igual ao e-mail do atleta 2.";
  if (!values.parceiro_genero)
    errors.parceiro_genero = "Selecione o gênero do atleta 2.";
      ...
      {ETAPAS.map((e, i) => {
        ...
        return (
          ...
          >
            {i > 0 && (
              <div
                className={`absolute left-0 top-3 h-px w-1/2 ${i <= idx ? "bg-blue-600" : "bg-gray-200"}`}
              />
            )}
            {i < ETAPAS.length - 1 && (
              <div
                className={`absolute right-0 top-3 h-px w-1/2 ${i < idx ? "bg-blue-600" : "bg-gray-200"}`}
              />
            )}
            <div
            ...
            <span
              className={`text-center text-[11px] font-medium leading-tight ${atual ? "text-blue-600" : "text-gray-400"}`}
            >
              {e.label}
            </span>
          </div>
    ...
    <section className="space-y-3 rounded-2xl bg-blue-50/60 p-4 ring-1 ring-blue-100">
      <div>
        ...
        <p className="text-xs text-gray-500">
          Este campeonato recomenda a categoria pelo nível — responda as 5
          perguntas abaixo.
        </p>
      </div>
      {PERGUNTAS_NIVEL.map((p) => (
        <div key={p.key}>
          ...
          <select
            ...
            onChange={(event) =>
              onChange(`${prefixo}${p.key}`, event.target.value)
            }
            required
          >
...
export function IngressoAtletaForm({
  ...
  const [etapa, setEtapa] = useState<Etapa>(
    reservedCategory ? "dados" : "categoria",
  );
  const [catSelecionada, setCat] = useState<CategoriaOpcao | null>(
    initialCategory,
  );
  const [reservation, setReservation] =
    useState<AthleteCheckoutReservation | null>(initialReservation ?? null);
  ...
  const [dismissedErrors, setDismissedErrors] = useState<
    Partial<Record<ComprarAtletaField, number>>
  >({});
  ...
  const valuesBeforeAccountAutofill = useRef<Record<string, string> | null>(
    null,
  );
  ...
  useEffect(() => {
    if (!checkoutDraftKey || !reservation || draftReadyKey !== checkoutDraftKey)
      return;
    try {
  ...
  const visibleFieldError = (field: ComprarAtletaField) =>
    reviewErrors[field] ??
    (dismissedErrors[field] === state.validationAttempt
      ? undefined
  ...
  function updateValue(field: string, value: string) {
    ...
    setReviewErrors((current) =>
      Object.keys(current).length > 0
        ? validateAthleteFields(nextValues, usarMesmoEmail)
        : current,
    );
  ...
  function updateAthleteEmail(field: AthleteEmailField, email: string) {
    const next = setAthleteEmail(values, field, email);
    const nextValues =
  ...
  function toggleMesmoEmail(checked: boolean) {
    ...
    setValues((current) => ({
      ...
      parceiro_email_confirmacao: checked
  ...
  function toggleDadosDaConta(checked: boolean) {
    ...
    setValues((current) => {
      ...
      const next: Record<string, string> = {
        ...current,
        comprador_nome:
          authenticatedAthlete.name || current.comprador_nome || "",
        comprador_cpf: authenticatedAthlete.cpf
          ? formatCpf(authenticatedAthlete.cpf)
          : current.comprador_cpf || "",
        comprador_zap:
          authenticatedAthlete.whatsapp || current.comprador_zap || "",
        comprador_email:
          authenticatedAthlete.email || current.comprador_email || "",
        comprador_email_confirmacao:
          authenticatedAthlete.email ||
          current.comprador_email_confirmacao ||
          "",
        comprador_genero:
          authenticatedAthlete.gender || current.comprador_genero || "",
        comprador_camisa:
          authenticatedAthlete.shirt || current.comprador_camisa || "",
      };
  ...
  function continuarComCategoria() {
    ...
    startReservationTransition(async () => {
      const result = await reservarCategoriaAtleta(
        championshipId,
        catSelecionada.id,
      );
      if (!result.ok) {
  ...
  function abrirRevisao() {
    const errors = validateAthleteFields(values, usarMesmoEmail);
    setReviewErrors(errors);
    const firstInvalid = errors.comprador_nome
      ? compradorNomeRef
      : errors.comprador_cpf
        ? compradorCpfRef
        : errors.comprador_email
          ? compradorEmailRef
          : errors.comprador_email_confirmacao
            ? compradorEmailConfirmacaoRef
            : errors.parceiro_nome
              ? parceiroNomeRef
              : errors.parceiro_cpf
                ? parceiroCpfRef
                : errors.parceiro_email
  ...
  const podeCompartilharEmail = (values.comprador_email ?? "")
    .trim()
    .includes("@");
    ...
    <div className="space-y-6 pb-28 sm:pb-24">
      ...
      >
        <div className="p-6 text-center">
          ...
          >
            A vaga foi liberada. Escolha novamente a categoria para iniciar uma
            nova reserva.
          </p>
          <button
            type="button"
      ...
      {etapa === "categoria" && (
        <div ref={categorySectionRef} className="scroll-mt-4 space-y-2">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div
                    className={`flex size-9 shrink-0 items-center justify-center rounded-lg ${sel && !cat.esgotado ? "bg-blue-600" : "bg-gray-100"}`}
                  >
                    <Trophy
                      className={`size-5 ${sel && !cat.esgotado ? "text-white" : "text-gray-400"}`}
                    />
                  </div>
                      ...
                      <>
                        {loteAtivo ? (
                          <p className="text-xs text-amber-600">
                            {loteAtivo.nome}
                            {loteAtivo.dataFim &&
                              ` · até ${new Date(loteAtivo.dataFim).toLocaleDateString("pt-BR")}`}
                          </p>
                        ) : null}
                        {cat.vagasDisponiveis === null ? (
                          ...
                          <p className="text-xs font-medium text-blue-600">
                            <span className="font-bold">
                              {cat.vagasDisponiveis}
                            </span>{" "}
                            {cat.vagasDisponiveis === 1
                              ? "vaga disponível"
                              : "vagas disponíveis"}
                          </p>
                        )}
          ...
          >
            ...
            {reservationPending
              ? "Reservando vaga…"
              : "Continuar com esta categoria"}
          </button>
          {reservationError && (
            <div
              className="flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-red-700"
              aria-live="polite"
            >
      ...
      {etapa !== "categoria" && catSelecionada && (
        <form action={formAction} noValidate className="space-y-6">
          ...
          <input
            type="hidden"
            name="usar_mesmo_email"
            value={usarMesmoEmail ? "1" : "0"}
          />
          {waitlistInviteToken && (
            <input
              type="hidden"
              name="waitlist_invite"
              value={waitlistInviteToken}
            />
          )}
          ...
          <div hidden={etapa !== "dados"} className="space-y-5">
            ...
            <section
              ...
              aria-labelledby="atleta-1-title"
            >
              <div className="flex items-center gap-3 border-b border-gray-100 bg-blue-50/70 px-4 py-3.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Dados da primeira pessoa da dupla
                  </p>
                </div>
              </div>
              <div className="space-y-4 p-4">
                {authenticatedAthlete && (
                  <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-blue-100 bg-blue-50/70 p-3.5">
                    <input
                      ...
                      onChange={(event) =>
                        toggleDadosDaConta(event.target.checked)
                      }
                      className="mt-0.5 size-4 shrink-0 accent-blue-600"
                    />
                    <span className="text-sm text-blue-950">
                      <span className="block font-semibold">
                        Você é um dos atletas?
                      </span>
                      <span className="mt-0.5 block text-xs text-blue-700">
                        Sim, sou o atleta 1. Preencher meus dados usando a conta
                        logada.
                      </span>
                    </span>
                  </label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      name="comprador_zap"
                      inputMode="numeric"
                      className={`mt-1 ${input} ${visibleFieldError("comprador_zap") ? "border-red-400 ring-1 ring-red-300 focus:ring-red-400" : ""}`}
                      placeholder="DDD + número"
                      value={values.comprador_zap ?? ""}
                      onChange={(event) =>
                        updateValue("comprador_zap", event.target.value)
                      }
                      aria-invalid={!!visibleFieldError("comprador_zap")}
                      aria-describedby={
                        visibleFieldError("comprador_zap")
                          ? "comprador-zap-error"
                          : undefined
                      }
                      autoComplete="tel"
                ...
                <div>
                  ...
                  {emailDaConta && (
                    <SugestaoEmailDaConta
                      ...
                      atleta="atleta 1"
                      onUse={() =>
                        updateAthleteEmail("comprador_email", emailDaConta)
                      }
                    />
                  )}
                  <p className="mt-1 text-xs text-gray-500">
                    Enviaremos o ingresso e o QR para este endereço.
                  </p>
                </div>
                {podeCompartilharEmail && (
                  <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-3">
                    <input
                      ...
                      onChange={(event) =>
                        toggleMesmoEmail(event.target.checked)
                      }
                    ...
                    <span>
                      ...
                      <span className="mt-0.5 block text-xs text-gray-500">
                        Os ingressos continuam individuais, mesmo na mesma caixa
                        de entrada.
                      </span>
                    </span>
                  </label>
                ...
                <div>
                  <label
                    htmlFor="comprador_email_confirmacao"
                    className="block text-sm font-medium text-gray-700"
                  >
                    Confirme seu e-mail
                  </label>
                  <input
                    ...
                    aria-describedby={
                      visibleFieldError("comprador_email_confirmacao")
                        ? "comprador-email-confirmacao-error"
                        : undefined
                    }
                    autoComplete="off"
                    required
                  ...
                  {visibleFieldError("comprador_email_confirmacao") && (
                    <p
                      id="comprador-email-confirmacao-error"
                      className="mt-1 text-xs font-medium text-red-600"
                    >
                      {visibleFieldError("comprador_email_confirmacao")}
                    </p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className={`mt-1 ${select} ${visibleFieldError("comprador_genero") ? "border-red-400 ring-1 ring-red-300 focus:ring-red-400" : ""}`}
                      value={values.comprador_genero ?? ""}
                      onChange={(event) =>
                        updateValue("comprador_genero", event.target.value)
                      }
                      aria-invalid={!!visibleFieldError("comprador_genero")}
                      aria-describedby={
                        visibleFieldError("comprador_genero")
                          ? "comprador-genero-error"
                          : undefined
                      }
                      required
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        updateValue("comprador_camisa", event.target.value)
                      }
                    >
            ...
            <section
              className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm"
              aria-labelledby="atleta-2-title"
            >
              <div className="flex items-center gap-3 border-b border-gray-100 bg-emerald-50/70 px-4 py-3.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Dados da segunda pessoa da dupla
                  </p>
                </div>
              </div>
              <div className="space-y-4 p-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateValue("parceiro_nome", event.target.value)
                    }
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      name="parceiro_zap"
                      inputMode="numeric"
                      className={`mt-1 ${input} ${visibleFieldError("parceiro_zap") ? "border-red-400 ring-1 ring-red-300 focus:ring-red-400" : ""}`}
                      placeholder="DDD + número"
                      value={values.parceiro_zap ?? ""}
                      onChange={(event) =>
                        updateValue("parceiro_zap", event.target.value)
                      }
                      aria-invalid={!!visibleFieldError("parceiro_zap")}
                      aria-describedby={
                        visibleFieldError("parceiro_zap")
                          ? "parceiro-zap-error"
                          : undefined
                      }
                      autoComplete="off"
                ...
                {!usarMesmoEmail ? (
                  <>
                    <div>
                      ...
                      {emailDaConta && (
                        <SugestaoEmailDaConta
                          ...
                          atleta="atleta 2"
                          onUse={() =>
                            updateAthleteEmail("parceiro_email", emailDaConta)
                          }
                        />
                      )}
                      <p className="mt-1 text-xs text-gray-500">
                    ...
                    <div>
                      <label
                        htmlFor="parceiro_email_confirmacao"
                        className="block text-sm font-medium text-gray-700"
                      >
                        Confirme o e-mail do parceiro
                      </label>
                      <input
                        ...
                        aria-describedby={
                          visibleFieldError("parceiro_email_confirmacao")
                            ? "parceiro-email-confirmacao-error"
                            : undefined
                        }
                        autoComplete="off"
                        required
                      ...
                      {visibleFieldError("parceiro_email_confirmacao") && (
                        <p
                          id="parceiro-email-confirmacao-error"
                          className="mt-1 text-xs font-medium text-red-600"
                        >
                          {visibleFieldError("parceiro_email_confirmacao")}
                        </p>
                  ...
                  <div className="rounded-xl bg-gray-50 px-4 py-3 text-sm text-gray-700 ring-1 ring-gray-200">
                    O ingresso do atleta 2 também será enviado para{" "}
                    <strong className="break-all">
                      {values.comprador_email}
                    </strong>
                    .
                    <input
                      type="hidden"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className={`mt-1 ${select} ${visibleFieldError("parceiro_genero") ? "border-red-400 ring-1 ring-red-300 focus:ring-red-400" : ""}`}
                      value={values.parceiro_genero ?? ""}
                      onChange={(event) =>
                        updateValue("parceiro_genero", event.target.value)
                      }
                      aria-invalid={!!visibleFieldError("parceiro_genero")}
                      aria-describedby={
                        visibleFieldError("parceiro_genero")
                          ? "parceiro-genero-error"
                          : undefined
                      }
                      required
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        updateValue("parceiro_camisa", event.target.value)
                      }
                    >
                      <option value="">Não informar</option>
                      {CAMISAS.map((t) => (
            ...
            {!isGratis && (
              <section className="space-y-3">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    Escolha antes de gerar a cobrança.
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {[
            ...
            <section className="space-y-5" aria-labelledby="revisao-title">
              <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
                ...
                <p className="mt-1 text-xs text-blue-700">
                  Cada ingresso e QR será enviado somente para o e-mail mostrado
                  abaixo.
                </p>
              </div>
              ...
              {usarMesmoEmail && (
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                  Você escolheu receber as duas credenciais no mesmo e-mail.
                  Quem tiver acesso a essa caixa poderá acessar e recuperar os
                  dois ingressos individuais.
                </div>
              )}
              ...
              <div className="divide-y divide-gray-100 rounded-2xl border border-gray-200 bg-white">
                <div className="p-4">
                  ...
                  <p className="mt-1 font-semibold text-gray-900">
                    {values.comprador_nome}
                  </p>
                  <p className="break-all text-sm text-blue-700">
                    {values.comprador_email?.trim().toLowerCase()}
                  </p>
                </div>
                <div className="p-4">
                  <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">
                  ...
                  <p className="mt-1 font-semibold text-gray-900">
                    {values.parceiro_nome}
                  </p>
                  <p className="break-all text-sm text-blue-700">
                    {values.parceiro_email?.trim().toLowerCase()}
                  </p>
                </div>
                ...
                <div className="space-y-2 p-4 text-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {catSelecionada.genero === "mista"
                          ? "Categoria mista"
                          : `Categoria ${catSelecionada.genero === "masculino" ? "masculina" : "feminina"}`}
                      </p>
                    ...
                    <p className="shrink-0 font-bold text-gray-900">
                      {isGratis ? "Grátis" : formatBRL(total)}
                    </p>
                  </div>
                  {!isGratis ? (
