...
function ContributionForm({ today, item, onDone }: { today: string; item?: InvestmentContribution; onDone: () => void }) { return <AsyncForm action={item ? (data) => editarAporteLifeOS(item.id, data) : criarAporteLifeOS} onDone={onDone}><div className="grid gap-3 sm:grid-cols-2"><Field name="amount" label="Valor" type="number" step="0.01" defaultValue={item?.amount.toFixed(2)} required /><Field name="date" label="Data" type="date" defaultValue={item?.date ?? today} required /></div><Field name="institution" label="Instituicao" defaultValue={item?.institution ?? undefined} /><Field name="notes" label="Observacao" defaultValue={item?.notes ?? undefined} /></AsyncForm>; }
function ChartCard({ title, description, children }: { title: string; description: string; children: React.ReactNode }) { return <section className="min-w-0 overflow-hidden rounded-lg border border-white/10 bg-[#15191f] p-4 text-white sm:p-5"><h2 className="font-semibold">{title}</h2><p className="mt-1 text-xs text-white/35">{description}</p><div className="mt-5 h-64 min-w-0">{children}</div></section>; }
