  ...
  return <section className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <ChartCard title="Evolucao mensal da carteira" description="Ultimo valor registrado em cada mes">
        {portfolioData.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={portfolioData} margin={{ top: 8, right: 10, left: 2, bottom: 0 }}><CartesianGrid vertical={false} stroke="#eef0f3" /><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#9ca3af" }} /><YAxis width={72} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#9ca3af" }} tickFormatter={(value) => compactMoney(Number(value))} /><Tooltip formatter={(value) => [formatBRL(Number(value)), "Carteira"]} contentStyle={{ borderRadius: 8, borderColor: "#e5e7eb" }} /><Line type="monotone" dataKey="value" stroke="#059669" strokeWidth={2.5} dot={{ r: 3 }} /></LineChart></ResponsiveContainer> : <EmptyChart text="Atualize o valor da carteira para formar a evolucao." />}
      </ChartCard>
      <ChartCard title="Aportes mensais" description="Quanto foi investido em cada mes">
        {contributionData.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={contributionData} margin={{ top: 8, right: 10, left: 2, bottom: 0 }}><CartesianGrid vertical={false} stroke="#eef0f3" /><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#9ca3af" }} /><YAxis width={72} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#9ca3af" }} tickFormatter={(value) => compactMoney(Number(value))} /><Tooltip formatter={(value) => [formatBRL(Number(value)), "Aportado"]} cursor={{ fill: "#f3f4f6" }} contentStyle={{ borderRadius: 8, borderColor: "#e5e7eb" }} /><Bar dataKey="value" fill="#2563eb" radius={[4, 4, 0, 0]} maxBarSize={44} /></BarChart></ResponsiveContainer> : <EmptyChart text="Registre aportes para comparar os meses." />}
      </ChartCard>
    </div>

    <section className="rounded-lg bg-white p-5 text-gray-900">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Movimentacoes da carteira</h2><p className="mt-1 text-xs text-gray-400">O valor real vem do Investidor10 e os aportes permanecem editaveis.</p></div><div className="flex gap-2"><button type="button" onClick={() => setNewSnapshot(true)} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white"><Wallet className="size-4" />Atualizar carteira</button><button type="button" onClick={() => setNewContribution(true)} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white"><Plus className="size-4" />Novo aporte</button></div></div>
      <div className="mt-5 divide-y divide-gray-100">{[...contributions].sort((a, b) => b.date.localeCompare(a.date)).map((item) => <div key={item.id} className="flex items-center gap-3 py-3"><div className="min-w-0 flex-1"><p className="font-semibold">{formatBRL(item.amount)}</p><p className="mt-0.5 text-xs text-gray-400">{formatDateBR(item.date)} · {item.institution ?? "Sem instituicao"}</p></div><button type="button" onClick={() => setEditing(item)} className="rounded-md p-1.5 text-gray-400 hover:bg-gray-100 hover:text-blue-600" title="Editar aporte"><Pencil className="size-4" /></button><button type="button" onClick={async () => { if (window.confirm("Excluir este aporte?")) { await removerAporteLifeOS(item.id); router.refresh(); } }} className="rounded-md p-1.5 text-gray-300 hover:bg-red-50 hover:text-red-500" title="Excluir aporte"><Trash2 className="size-4" /></button></div>)}{!contributions.length && <p className="py-8 text-center text-sm text-gray-400">Nenhum aporte registrado.</p>}</div>
    </section>
...
function ChartCard({ title, description, children }: { title: string; description: string; children: React.ReactNode }) { return <section className="rounded-lg bg-white p-5 text-gray-900"><h2 className="font-semibold">{title}</h2><p className="mt-1 text-xs text-gray-400">{description}</p><div className="mt-5 h-64">{children}</div></section>; }
function EmptyChart({ text }: { text: string }) { return <div className="flex h-full items-center justify-center rounded-lg bg-gray-50 px-5 text-center text-sm text-gray-400">{text}</div>; }
...
function Field({ name, label, type = "text", defaultValue, required, step }: { name: string; label: string; type?: string; defaultValue?: string; required?: boolean; step?: string }) { return <label className="block text-xs font-medium text-gray-500">{label}<input name={name} type={type} step={step} defaultValue={defaultValue} required={required} className={`${inputClass} mt-1`} /></label>; }
