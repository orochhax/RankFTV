...
import { eventIntersectsDate, expandEventOccurrences, type EventRecurrenceRule } from "@/lib/event-recurrence";

export type CalendarEvent = { id: string; title: string; description: string | null; startAt: string; endAt: string; allDay: boolean; status: string; location: string | null; recurrenceRule?: EventRecurrenceRule | null; recurrenceGroupId?: string | null; occurrenceId?: string; baseStartAt?: string; baseEndAt?: string };
...
export function CalendarClient({ events, embedded = false, initialDate }: { events: CalendarEvent[]; embedded?: boolean; initialDate: string }) {
  ...
  const visible = useMemo(() => {
    if (mode === "year") return expandEventOccurrences(events, { from: `${date.slice(0, 4)}-01-01`, to: `${date.slice(0, 4)}-12-31` });
    if (mode === "month") { const first = `${date.slice(0, 7)}-01`; const offset = parseDate(first).getDay() === 0 ? 6 : parseDate(first).getDay() - 1; const from = shift(first, -offset); return expandEventOccurrences(events, { from, to: shift(from, 41) }); }
    if (mode === "week") { const start = monday(date); return expandEventOccurrences(events, { from: start, to: shift(start, 6) }); }
    return expandEventOccurrences(events, { from: date, to: date });
  }, [date, events, mode]);
...
function CalendarBody({ mode, date, today, events, onDay, onEvent, onOpenMonth }: { mode: CalendarMode; date: string; today: string; events: CalendarEvent[]; onDay: (date: string) => void; onEvent: (event: CalendarEvent) => void; onOpenMonth: (month: string) => void }) {
  ...
  if (mode === "week") { const start = monday(date); return <section className="mt-5 overflow-x-auto rounded-lg border border-white/10 bg-[#15191f] p-3 text-white"><div className="grid min-w-[840px] grid-cols-7 divide-x divide-white/10">{Array.from({ length: 7 }, (_, index) => { const current = shift(start, index); const dayEvents = events.filter((event) => eventIntersectsDate(event, current)); return <div key={current} className="min-h-[420px] px-2"><button type="button" onClick={() => onDay(current)} className={`mb-3 w-full rounded-lg px-2 py-3 text-center hover:bg-white/[0.06] ${current === today ? "bg-blue-600 text-white" : "bg-white/[0.03] text-white/60"}`}><span className="block text-[10px] uppercase">{new Intl.DateTimeFormat("pt-BR", { weekday: "short", timeZone: "UTC" }).format(new Date(`${current}T12:00:00Z`))}</span><b className="mt-1 block text-lg">{Number(current.slice(8))}</b></button><div className="space-y-2">{dayEvents.map((event) => <EventCard key={`${event.id}-${current}`} event={event} onClick={() => onEvent(event)} compact />)}</div></div>; })}</div></section>; }
    ...
    <div className="min-w-[700px]">
      ...
      <div className="grid grid-cols-7">
        {Array.from({ length: 42 }, (_, index) => {
          ...
          const dayEvents = events.filter((event) => eventIntersectsDate(event, current));
          return <div key={current} className={`min-h-28 border-b border-r border-white/10 p-2 ${inMonth ? "bg-[#15191f] text-white/75" : "bg-black/10 text-white/20"}`}>
            ...
            <div className="mt-1 space-y-1">
              {dayEvents.slice(0, 3).map((event) => <button key={`${event.occurrenceId ?? event.id}-${event.startAt}`} type="button" onClick={() => onEvent(event)} className="block w-full truncate rounded bg-blue-400/10 px-1.5 py-1 text-left text-[10px] font-medium text-blue-200"><span className="mr-1 text-blue-400">{event.allDay ? "Dia inteiro" : eventTime(event.startAt)}</span>{event.title}</button>)}
...
function EventModal({ date, event, onClose, onDone }: { date: string; event?: CalendarEvent; onClose: () => void; onDone: () => void }) {
  ...
  const baseStart = event?.baseStartAt ?? event?.startAt; const baseEnd = event?.baseEndAt ?? event?.endAt;
  const startDate = baseStart ? eventDate(baseStart) : date; const endDate = baseEnd ? (event?.allDay ? shift(eventDate(baseEnd), -1) : eventDate(baseEnd)) : date;
  const [recurrenceAnchor, setRecurrenceAnchor] = useState(startDate);
  const submit = (formData: FormData) => {
    ...
    if (formData.get("all_day") !== "on" && new Date(ends).getTime() <= new Date(starts).getTime()) return setError("O termino precisa ser depois do inicio.");
  ...
  const remove = async () => {
    if (!event) return;
    const approved = await confirm({ title: event.recurrenceRule ? "Excluir série recorrente?" : "Excluir evento?", description: event.recurrenceRule ? `Todas as ocorrências de “${event.title}” serão removidas.` : `O evento “${event.title}” será removido da agenda definitivamente.`, confirmLabel: event.recurrenceRule ? "Excluir série" : "Excluir evento" });
    if (!approved) return;
