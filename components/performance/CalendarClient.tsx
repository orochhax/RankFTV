...
export function CalendarClient({ events, embedded = false, initialDate }: { events: CalendarEvent[]; embedded?: boolean; initialDate: string }) {
  ...
  const content = <>
    ...
    <div className="mt-3 flex gap-1">{(["day", "week", "month", "year"] as CalendarMode[]).map((value) => <button type="button" key={value} onClick={() => setMode(value)} className={`rounded-lg px-3 py-2 text-sm ${mode === value ? "bg-white text-gray-900" : "text-white/55 hover:bg-white/10"}`}>{value === "day" ? "Dia" : value === "week" ? "Semana" : value === "month" ? "Mes" : "Ano"}</button>)}</div>
  ...
  return <>{embedded ? <section className="text-white">{content}</section> : <main className="min-h-screen bg-[#0b0d10] px-4 py-5 text-white sm:px-6"><div className="mx-auto max-w-7xl">{content}</div></main>}{draftDate && <EventModal date={draftDate} onClose={() => setDraftDate(null)} onDone={() => { setDraftDate(null); router.refresh(); }} />}{editing && <EventModal event={editing} date={eventDate(editing.startAt)} onClose={() => setEditing(null)} onDone={() => { setEditing(null); router.refresh(); }} />}</>;
}
...
function CalendarBody({ mode, date, today, events, onDay, onEvent, onOpenMonth }: { mode: CalendarMode; date: string; today: string; events: CalendarEvent[]; onDay: (date: string) => void; onEvent: (event: CalendarEvent) => void; onOpenMonth: (month: string) => void }) {
  if (mode === "day") return <section className="mt-5 rounded-lg bg-white p-5 text-gray-900"><button type="button" onClick={() => onDay(date)} className="flex w-full items-center justify-between rounded-lg bg-gray-50 px-3 py-2 text-left"><span className="font-semibold">{formatDateBR(date)}</span><span className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600"><Plus className="size-3.5" />Evento</span></button><div className="mt-3 space-y-2">{events.map((event) => <EventCard key={event.id} event={event} onClick={() => onEvent(event)} />)}{!events.length && <EmptyDay onClick={() => onDay(date)} />}</div></section>;
  if (mode === "week") { const start = monday(date); return <section className="mt-5 overflow-x-auto rounded-lg bg-white p-3 text-gray-900"><div className="grid min-w-[840px] grid-cols-7 divide-x divide-gray-100">{Array.from({ length: 7 }, (_, index) => { const current = shift(start, index); const dayEvents = events.filter((event) => eventDate(event.startAt) === current); return <div key={current} className="min-h-[420px] px-2"><button type="button" onClick={() => onDay(current)} className={`mb-3 w-full rounded-lg px-2 py-3 text-center hover:bg-blue-50 ${current === today ? "bg-blue-600 text-white" : "bg-gray-50"}`}><span className="block text-[10px] uppercase">{new Intl.DateTimeFormat("pt-BR", { weekday: "short", timeZone: "UTC" }).format(new Date(`${current}T12:00:00Z`))}</span><b className="mt-1 block text-lg">{Number(current.slice(8))}</b></button><div className="space-y-2">{dayEvents.map((event) => <EventCard key={event.id} event={event} onClick={() => onEvent(event)} compact />)}</div></div>; })}</div></section>; }
  if (mode === "year") return <section className="mt-5 grid gap-3 sm:grid-cols-3 lg:grid-cols-4">{Array.from({ length: 12 }, (_, index) => { const month = `${date.slice(0, 4)}-${String(index + 1).padStart(2, "0")}`; const count = events.filter((event) => eventDate(event.startAt).slice(0, 7) === month).length; return <button type="button" key={month} onClick={() => onOpenMonth(month)} className="rounded-lg bg-white p-4 text-left text-gray-900 hover:ring-2 hover:ring-blue-500"><p className="font-semibold capitalize">{new Intl.DateTimeFormat("pt-BR", { month: "long", timeZone: "UTC" }).format(new Date(`${month}-01T12:00:00Z`))}</p><p className="mt-3 text-xs text-gray-400">{count} evento{count === 1 ? "" : "s"}</p></button>; })}</section>;

  const first = `${date.slice(0, 7)}-01`; const offset = parseDate(first).getDay() === 0 ? 6 : parseDate(first).getDay() - 1; const gridStart = shift(first, -offset);
  return <section className="mt-5 overflow-hidden rounded-lg bg-white text-gray-900"><div className="grid grid-cols-7 border-b border-gray-100 bg-gray-50 text-center text-xs font-semibold text-gray-400">{["Seg", "Ter", "Qua", "Qui", "Sex", "Sab", "Dom"].map((label) => <span key={label} className="py-3">{label}</span>)}</div><div className="grid grid-cols-7">{Array.from({ length: 42 }, (_, index) => { const current = shift(gridStart, index); const inMonth = current.slice(0, 7) === date.slice(0, 7); const dayEvents = events.filter((event) => eventDate(event.startAt) === current); return <button type="button" key={current} onClick={() => onDay(current)} className={`min-h-28 border-b border-r border-gray-100 p-2 text-left align-top hover:bg-blue-50/50 ${inMonth ? "bg-white" : "bg-gray-50/60 text-gray-300"}`}><span className={`inline-flex size-7 items-center justify-center rounded-full text-xs font-semibold ${current === today ? "bg-blue-600 text-white" : ""}`}>{Number(current.slice(8))}</span><span className="mt-1 block space-y-1">{dayEvents.slice(0, 3).map((event) => <span key={event.id} role="button" tabIndex={0} onClick={(click) => { click.stopPropagation(); onEvent(event); }} onKeyDown={(key) => { if (key.key === "Enter") { key.stopPropagation(); onEvent(event); } }} className="block truncate rounded bg-blue-50 px-1.5 py-1 text-[10px] font-medium text-blue-700"><span className="mr-1 text-blue-400">{eventTime(event.startAt)}</span>{event.title}</span>)}{dayEvents.length > 3 && <span className="block px-1 text-[10px] text-gray-400">+{dayEvents.length - 3} eventos</span>}</span></button>; })}</div></section>;
}

function EventCard({ event, onClick, compact = false }: { event: CalendarEvent; onClick: () => void; compact?: boolean }) { return <button type="button" onClick={onClick} className={`w-full rounded-lg border border-blue-100 bg-blue-50 text-left text-blue-900 hover:border-blue-300 ${compact ? "p-2" : "p-3"}`}><div className="flex items-start gap-2"><Clock3 className="mt-0.5 size-3.5 shrink-0 text-blue-500" /><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold">{event.title}</span><span className="mt-0.5 block text-[10px] text-blue-500">{eventTime(event.startAt)} - {eventTime(event.endAt)}</span></span><Pencil className="size-3.5 shrink-0 text-blue-300" /></div>{event.location && !compact && <span className="mt-2 flex items-center gap-1 text-[10px] text-blue-500"><MapPin className="size-3" />{event.location}</span>}</button>; }
function EmptyDay({ onClick }: { onClick: () => void }) { return <button type="button" onClick={onClick} className="flex w-full items-center justify-center gap-2 rounded-lg border border-dashed border-gray-200 p-8 text-sm text-gray-400 hover:border-blue-300 hover:text-blue-600"><CalendarDays className="size-4" />Clique para adicionar um evento</button>; }
...
function EventModal({ date, event, onClose, onDone }: { date: string; event?: CalendarEvent; onClose: () => void; onDone: () => void }) {
  ...
  const remove = () => startTransition(async () => { if (!event || !window.confirm("Excluir este evento?")) return; const result = await removerEventoLifeOS(event.id); if (!result.ok) setError(result.error ?? "Nao foi possivel excluir."); else onDone(); });
  return <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/60 p-3 sm:items-center"><div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white p-5 text-gray-900"><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-bold">{event ? "Editar evento" : "Novo evento"}</h2><button type="button" onClick={onClose} className="text-sm text-gray-500">Fechar</button></div><form action={submit} className="space-y-3"><Field name="title" label="Nome" defaultValue={event?.title} required /><div className="grid grid-cols-2 gap-3"><Field name="start_date" label="Data de inicio" type="date" defaultValue={startDate} required /><Field name="start_time" label="Hora de inicio" type="time" defaultValue={event ? eventTime(event.startAt) : "09:00"} required /><Field name="end_date" label="Data de termino" type="date" defaultValue={endDate} required /><Field name="end_time" label="Hora de termino" type="time" defaultValue={event ? eventTime(event.endAt) : "10:00"} required /></div><Field name="location" label="Local" defaultValue={event?.location ?? undefined} /><label className="block text-xs font-medium text-gray-500">Descricao<textarea name="description" defaultValue={event?.description ?? undefined} rows={3} className={`${inputClass} mt-1 resize-none`} /></label>{error && <p className="text-xs text-red-600">{error}</p>}<div className="flex items-center justify-between gap-3"><button type="submit" disabled={pending} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{pending ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />}Salvar</button>{event && <button type="button" onClick={remove} disabled={pending} className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold text-red-600 hover:bg-red-50"><Trash2 className="size-4" />Excluir</button>}</div></form></div></div>;
}
