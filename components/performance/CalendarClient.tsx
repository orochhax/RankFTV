...
function CalendarBody({ mode, date, today, events, onDay, onEvent, onOpenMonth }: { mode: CalendarMode; date: string; today: string; events: CalendarEvent[]; onDay: (date: string) => void; onEvent: (event: CalendarEvent) => void; onOpenMonth: (month: string) => void }) {
  ...
  return <section className="mt-5 overflow-hidden rounded-lg border border-white/10 bg-[#15191f] text-white"><div className="grid grid-cols-7 border-b border-white/10 bg-white/[0.025] text-center text-xs font-semibold text-white/35">{["Seg", "Ter", "Qua", "Qui", "Sex", "Sab", "Dom"].map((label) => <span key={label} className="py-3">{label}</span>)}</div><div className="grid grid-cols-7">{Array.from({ length: 42 }, (_, index) => { const current = shift(gridStart, index); const inMonth = current.slice(0, 7) === date.slice(0, 7); const dayEvents = events.filter((event) => eventDate(event.startAt) === current); return <button type="button" key={current} onClick={() => onDay(current)} className={`min-h-28 border-b border-r border-white/10 p-2 text-left align-top hover:bg-white/[0.05] ${inMonth ? "bg-[#15191f] text-white/75" : "bg-black/10 text-white/20"}`}><span className={`inline-flex size-7 items-center justify-center rounded-full text-xs font-semibold ${current === today ? "bg-blue-600 text-white" : ""}`}>{Number(current.slice(8))}</span><span className="mt-1 block space-y-1">{dayEvents.slice(0, 3).map((event) => <span key={event.id} role="button" tabIndex={0} onClick={(click) => { click.stopPropagation(); onEvent(event); }} onKeyDown={(key) => { if (key.key === "Enter") { key.stopPropagation(); onEvent(event); } }} className="block truncate rounded bg-blue-400/10 px-1.5 py-1 text-[10px] font-medium text-blue-200"><span className="mr-1 text-blue-400">{eventTime(event.startAt)}</span>{event.title}</span>)}{dayEvents.length > 3 && <span className="block px-1 text-[10px] text-white/35">+{dayEvents.length - 3} eventos</span>}</span></button>; })}</div></section>;
}
