...
async function findPerformanceOwnerId(
  ...
  for (let page = 1; page <= 100; page += 1) {
    ...
    const owner = data.users.find(
      (user) => user.email?.trim().toLocaleLowerCase("pt-BR") === ownerEmail,
    );
    if (owner) {
      ...
      if (!ceoProfile)
        return {
          id: null,
          error: "A conta definida em ADMIN_EMAIL nao possui o papel CEO.",
        };
      return { id: owner.id, error: null };
    }
  ...
  return {
    id: null,
    error: "A conta definida em ADMIN_EMAIL nao foi encontrada.",
  };
}
...
export async function GET(request: NextRequest) {
  ...
  if (!owner.id)
    return NextResponse.json({ error: owner.error }, { status: 500 });
  ...
  try {
    ...
    results.push({
      userId: owner.id,
      state: result.state,
      mode: result.analysis.generation.mode,
    ...
    results.push({
      ...
      error:
        generationError instanceof Error
          ? generationError.message.slice(0, 200)
          : "Unknown error",
    });
  }
  ...
  return NextResponse.json(
    {
      ...
      generated: results.filter(
        (result) => result.state === "generated" || result.state === "updated",
      ).length,
