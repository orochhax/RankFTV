...
export type ReserveAthleteCategoryResult =
  ({ ok: true } & AthleteCheckoutReservation) | { ok: false; error: string };
...
export async function reservarCategoriaAtleta(
  ...
  if (!parsed.success)
    return { ok: false, error: "Campeonato ou categoria inválidos." };
  ...
  if (!(await checkRateLimit(`athlete-reservation:${ip}`, 20, 600))) {
    return {
      ok: false,
      error: "Muitas tentativas de reserva. Aguarde alguns minutos.",
    };
  }
  ...
  if (
    result.error?.message.includes(
      "checkout_reservation_category_change_required",
    )
  ) {
    ...
    result.error?.message.includes("checkout_reservation_token_consumed")
  ) {
  ...
  if (result.error || !reservation) {
    ...
    return {
      ok: false,
      error: checkoutReservationErrorMessage(
        result.error?.message ?? "reservation_invalid_response",
      ),
    };
  }
...
export async function expirarReservaCategoriaAtleta(
  championshipId: string,
): Promise<void> {
...
function calcularRatingDoFormulario(
  formData: FormData,
  prefixo: string,
): number | null {
...
export async function comprarIngressoAtleta(
  ...
  const metodoPagamento = parseAthleteTicketPaymentChoice(
    formData.get("metodo_pagamento"),
  );
  ...
  if (!metodoPagamento)
    return { error: "Selecione Pix ou cartão para continuar." };

  // Comprador
  const nome = ((formData.get("comprador_nome") as string) ?? "").trim();
  const cpf = normalizeCpf(formData.get("comprador_cpf") as string);
  const email = ((formData.get("comprador_email") as string) ?? "")
    .trim()
    .toLowerCase();
  const emailConfirmacao = (
    (formData.get("comprador_email_confirmacao") as string) ?? ""
  )
    .trim()
    .toLowerCase();
  const zap =
    ((formData.get("comprador_zap") as string) ?? "").replace(/\D/g, "") ||
    null;
  ...
  const pEmail =
    ((formData.get("parceiro_email") as string) ?? "").trim().toLowerCase() ||
    null;
  const pEmailConfirmacao = (
    (formData.get("parceiro_email_confirmacao") as string) ?? ""
  )
    .trim()
    .toLowerCase();
  const pZap =
    ((formData.get("parceiro_zap") as string) ?? "").replace(/\D/g, "") || null;
  ...
  if (!isValidAthleteName(nome) || nome.length < 2) {
  ...
  if (zap && !/^\d{10,15}$/.test(zap)) {
    fieldErrors.comprador_zap = "Informe o WhatsApp com DDD e número.";
  }
  if (!email || !email.includes("@")) {
    fieldErrors.comprador_email =
      "Informe um e-mail válido para acessar o ingresso.";
  }
  if (!emailConfirmacao || emailConfirmacao !== email) {
    fieldErrors.comprador_email_confirmacao =
      "A confirmação precisa ser igual ao e-mail do atleta 1.";
  }
  if (!genero) {
    fieldErrors.comprador_genero = "Selecione o gênero do atleta 1.";
  }
  if (!isValidAthleteName(pNome) || pNome.length < 2) {
    fieldErrors.parceiro_nome = pNome
      ? "Informe o nome do parceiro, não o e-mail."
      : "Informe o nome do parceiro.";
  }
  if (!validaCPF(pCpf)) {
    fieldErrors.parceiro_cpf =
  ...
  const [{ data: champ }, { data: cat }, { data: buyerProfile }] =
    await Promise.all([
      supabase
        .from("championships")
  ...
  const motorLigado = categoryLevelRecommendationEnabled(
    champ.usa_motor_categoria,
  );
  ...
  if (motorLigado) {
    ...
    if (compradorRating === null)
      return {
        error:
          "Responda as 5 perguntas de nível do atleta 1 (você) antes de continuar.",
      };
    parceiroRating = calcularRatingDoFormulario(formData, "parceiro_quiz_");
    if (parceiroRating === null)
      return {
        error:
          "Responda as 5 perguntas de nível do parceiro antes de continuar.",
      };
  }
  ...
  if (!elegibilidadeComprador.ok)
    return { error: `Você (atleta 1): ${elegibilidadeComprador.error}` };
  ...
  if (!elegibilidadeParceiro.ok)
    return { error: `Parceiro: ${elegibilidadeParceiro.error}` };
  ...
  if (cupomCodigo) {
    const { cupom, error: cupomErro } = await buscarCupomValido(
      championshipId,
      cupomCodigo,
      "atleta",
    );
  ...
  if (!isCheckoutReservationToken(reservationToken)) {
    return {
      error:
        "Sua reserva não foi encontrada. Volte à categoria e reserve a vaga novamente.",
    };
  }
  ...
  const { data: reservation } = await supabase
    ...
    .select(
      "id, championship_id, category_id, pricing_tier_id, price_snapshot, status, expires_at",
    )
  ...
  if (!reservation) {
    ...
    return {
      error:
        "O tempo da reserva terminou. Volte à categoria para tentar reservar novamente.",
    };
  }
  ...
  if (!Number.isFinite(valorReservado) || valorReservado < 0) {
    return {
      error:
        "Não foi possível validar o preço reservado. Tente reservar novamente.",
    };
  }
  ...
  if (valorReservado > 0) {
    ...
    if (!org?.chave_pix)
      return {
        error:
          "O organizador ainda não ativou o recebimento. Tente mais tarde.",
      };
  }
  ...
  if (cupomPreview) {
    const desconto = calcularDesconto(
      valorFinal,
      cupomPreview.tipoDesconto,
      cupomPreview.valorDesconto,
    );
    valorFinal = Math.round((valorFinal - desconto) * 100) / 100;
    const { data: claimed } = await supabase.rpc("claim_coupon_use", {
      p_coupon_id: cupomPreview.id,
    });
    if (!claimed) {
      return {
        error: "Esse cupom acabou de esgotar. Tente novamente sem ele.",
      };
    }
  ...
  async function liberarReservaECupom() {
    if (cupomId)
      await supabase.rpc("release_coupon_use", { p_coupon_id: cupomId });
    await supabase.rpc("release_athlete_checkout_reservation", {
      p_token_hash: reservationHash,
      p_force: true,
    });
  ...
  if (insErr || !ticket) {
    ...
    if (insErr?.message.includes("checkout_reservation_expired")) {
      return {
        error:
          "O tempo da reserva terminou. Volte à categoria para tentar novamente.",
      };
    }
