...
export default async function PerformancePage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [studyItemsRes, studyItemReferenceDetailsRes, studyModulesRes] = await Promise.all([
    ...
    supabase.from("perf_study_roadmap_item").select("id, module_id, requirements, workspace, preparation_steps, instructions, practice_exercises, reflection_questions, completion_checklist, evidence_prompt, completion_criteria, resource_title, resource_url, resource_channel").eq("user_id", user.id).eq("roadmap_id", loadedRoadmapId),
  ...
  const studyItemDetailsRes = studyItemReferenceDetailsRes.error
    ? await supabase.from("perf_study_roadmap_item").select("id, module_id, requirements, workspace, instructions, completion_criteria, resource_title, resource_url, resource_channel").eq("user_id", user.id).eq("roadmap_id", loadedRoadmapId)
    : null;
  const legacyStudyItemDetailsRes = studyItemReferenceDetailsRes.error && studyItemDetailsRes?.error
  ...
  const studyEnhancementsReady = (!studyItemReferenceDetailsRes.error || !studyItemDetailsRes?.error) && !enhancedQuestionError;
  ...
  const studyGenerationJobs: RoadmapGenerationJob[] = (studyGenerationJobsRes.data ?? []).flatMap((row) => row.status === "generating" || row.status === "failed" ? [{
    generationId: row.id,
    status: row.status,
  ...
  return <PerformanceConfirmProvider><LifeOSDashboard
    today={today}
    monday={monday}
    userId={user.id}
    email={user.email ?? ""}
    ...
    studyItems={(studyItemsRes.data ?? []).map((row) => { const details = itemDetailsById.get(row.id) as (typeof studyItemDetailRows)[number] & { requirements?: string | null; workspace?: string | null; preparation_steps?: unknown; practice_exercises?: unknown; reflection_questions?: unknown; completion_checklist?: unknown; evidence_prompt?: string | null } | undefined; return { id: row.id, roadmapId: row.roadmap_id, moduleId: details?.module_id ?? null, section: row.section, title: row.title, description: row.description, requirements: details?.requirements ?? null, workspace: details?.workspace ?? null, preparationSteps: stringList(details?.preparation_steps), instructions: details?.instructions ?? null, practiceExercises: stringList(details?.practice_exercises), reflectionQuestions: stringList(details?.reflection_questions), completionChecklist: stringList(details?.completion_checklist), evidence: details?.evidence_prompt ?? null, completionCriteria: details?.completion_criteria ?? null, resourceTitle: details?.resource_title ?? null, resourceUrl: details?.resource_url ?? null, resourceChannel: details?.resource_channel ?? null, orderIndex: Number(row.order_index), estimatedMinutes: asNumber(row.estimated_minutes), status: row.status, completedAt: row.completed_at, scheduledDate: row.scheduled_date, itemKind: row.item_kind }; })}
